...
def update_value(
    value: schemas.ValueCreate, value_id: int, db: Session = Depends(get_db)
):
...
def update_principle(
    principle: schemas.PrincipleCreate, principle_id: int, db: Session = Depends(get_db)
):
    db_principle = crud.update_principle(
        db, principle_id=principle_id, principle=principle
    )
    if db_principle is None:
        raise HTTPException(status_code=404, detail="Principle not found")
    return db_principle
